			...
			<div class="flex-1">
				<nav class="grid items-start px-2 text-sm font-medium lg:px-4">
					{#each dashboardPages as p}
						{#if p.visible}
							<a href={p.href} class="flex items-center gap-3 rounded-lg px-3 py-2 text-muted-foreground transition-all hover:text-primary" class:bg-muted={p.href === $page.url.pathname} class:text-primary={p.href === $page.url.pathname}>
			...
			<Sheet.Root>
				...
				<Sheet.Content side="left" class="flex flex-col">
					<nav class="grid gap-2 text-lg font-medium">
						...
						{#each dashboardPages as p}
							{#if p.visible}
								<a href={p.href} class="mx-[-0.65rem] flex items-center gap-4 rounded-xl px-3 py-2 hover:text-foreground" class:bg-muted={p.href === $page.url.pathname} class:text-muted-foreground={p.href !== $page.url.pathname} class:text-foreground={p.href === $page.url.pathname}>
									<svelte:component this={p.icon} class="h-4 w-4" />
									{p.name}
								</a>
							{/if}
